                    ...
                    <div>
                       {elem.colorsAndImg.map((item,index)=>{
                           ...
                           if (color[0]==this.props.item[1]){
                               ...
                               return  <img src={item[color[0]]} alt="item1" key={index} />
                           }
                       })} 
                    </div>
            <button className={shopingcartstyle.delete} onClick={this.deleteItem.bind(this.props.deleteid)}>X</button>
               
            <span className={shopingcartstyle.item_name}>{elem.name+" "+this.props.item[2]+" "+this.props.item[1]}</span>
           
            <span className={shopingcartstyle.item_price}>${elem.configurationAndPrice.map((elem)=>{
